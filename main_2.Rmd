...
df <- df %>%
  mutate(ln_car = log(Particulier),
         ...
         ev_share = critairE / Particulier,
         logit_ev_share = log(ev_share / (1 - ev_share)),
         fuel_elec_p_ratio = fuel_price / elec_price,
         ln_fuel_elec_p_ratio = log(fuel_price / elec_price)
         )
```
...
```{r}
df_lag <- df %>% 
  mutate(lag_ln_car = lag(ln_car),
         lag_logit_ev_share = lag(logit_ev_share))
df_lag <- df_lag[-1,]
```
...
```{r}
share_model <- lm(
  logit_ev_share ~ ln_income_per_capita + ln_fuel_elec_p_ratio,
  data = df
)
...
```{r}
share_model <- lm(
  logit_ev_share ~ ln_income_per_capita + year,
  data = df
)
